...
/* $(".btnDelete").click(function (e) { 
    let borrado = SELECCIONADOS.find(vehiculos => vehiculos.id == e.target.id);
  borrado.restarCantidad();
  $("#notificacion").empty();
    for (const vehiculos of SELECCIONADOS) {
        $("#notificacion").append(`<h3>${vehiculos.title}- Precio ${vehiculos.precios} Total: ${vehiculos.getTotal()}</h3>
        ` ); 
      }
    $("#notificacion").append(`  <b> Total General: ${calcularTotal(SELECCIONADOS)} </b>`);
  }); */
